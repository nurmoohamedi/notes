...
const Tag = ({ tag, deleteTag, note }) => {
  return (
    <a className="tag">
      {tag}
      <span className="tag_delete" onClick={() => deleteTag(note.id, tag)}>
        x
      </span>
    </a>
  );
      ...
      <div className="note__tag">
        {tagMode ? (
          <div className="note__tag_add">
            <label htmlFor="tag" data-text="text">
              #
            </label>
            <input
              type="text"
              id="tag"
              placeholder="enter tag for example #cool"
              value={tagValue}
          ...
          <div className="note__tag_tags">
            <div className="tags">
              {note?.tags && Array.isArray(note?.tags) && note?.tags.map((tag) => {
                return (
                  <Tag tag={tag} key={tag} deleteTag={deleteTag} note={note} />
                );
              })}
            </div>
